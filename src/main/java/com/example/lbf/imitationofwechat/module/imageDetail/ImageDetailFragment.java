 ...
 */
public class ImageDetailFragment extends BaseFragment implements ImageDetailContract.View, View.OnClickListener, ZoomImageView.OnSingleTapListener {
    ...
    private void initView() {
        ...
        if(selectMode == ChooseImageActivity.MODE_SINGLE){
            cbChooseImage.setVisibility(View.GONE);
            return;
        }
...
//                    选择图片满九张时通过返回true拦截点击事件
        ...
        cbChooseImage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String path = adapter.getImagePath(mViewPager.getCurrentItem());
                if(cbChooseImage.isChecked()){
...
//                                注意这里要设置immersive模式才能在隐藏status bar的情况下切换图片
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btSend:
                if(selectMode == ChooseImageActivity.MODE_MULTIPLE && selectedImageList.size()<9
                    ||selectMode == ChooseImageActivity.MODE_SINGLE){
//                    如果是单选模式或者多选模式且选择图片数少于9张，则把当前页的图片加入
                    selectedImageList.add(adapter.getImagePath(mViewPager.getCurrentItem()));
                }
